...
const Coupon = () => {
  ...
  const {data:couponsData} = useCouponsQuery({
    page,
    limit: 99,
  });
  const coupons = couponsData?.pages[0]?.data?.filter((x:any)=>x.vendor_type == (me?.business_profile[Object.keys(me?.business_profile)[0]]?.customer_type || orderBy(me?.shops,["customer_type"],["desc"])?.[0]?.customer_type || 1))?.map(x=>x.code);
  ...
  function onSubmit(value: any) {
    if(!coupons?.includes(value?.code))
    {
      alert("Invalid Code");
      return;
    }
    let couponObj = couponsData?.pages[0]?.data?.find(x=>x.code == value?.code);
    let orderListWithCode = orders?.filter(x=>x.coupon_id == couponObj?.id);
    if(orderListWithCode?.length >= couponObj?.usagePerUser)
    {
      alert("Invalid Code");
      return;
    }
