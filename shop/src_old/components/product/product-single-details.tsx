...
const ProductSingleDetails: React.FC<Props> = ({ product }: any) => {
  ...
  let stoneAttribute = [];
    ...
    let minMetalRate = metalAttribute[0]?.values?.find((z:any)=>z.value == min_price_metal_attribute?.value && z.vendor_type == (me?.business_profile[Object.keys(me?.business_profile)[0]]?.customer_type || orderBy(me?.shops,["customer_type"],["desc"])?.[0]?.customer_type || 1))?.rate;
    let minMetalRate24 = metalAttribute[0]?.values?.find((z:any)=>(z.value.includes("24")) && z.vendor_type == (me?.business_profile[Object.keys(me?.business_profile)[0]]?.customer_type || orderBy(me?.shops,["customer_type"],["desc"])?.[0]?.customer_type || 1))?.rate;
    let minStoneRate = stoneAttribute[0]?.values?.find((z:any)=>z.value == min_price_stone_attribute?.value && z.vendor_type == (me?.business_profile[Object.keys(me?.business_profile)[0]]?.customer_type || orderBy(me?.shops,["customer_type"],["desc"])?.[0]?.customer_type || 1))?.rate;
    let maxMetalRate = metalAttribute[0]?.values?.find((z:any)=>z.value == max_price_metal_attribute?.value && z.vendor_type == (me?.business_profile[Object.keys(me?.business_profile)[0]]?.customer_type || orderBy(me?.shops,["customer_type"],["desc"])?.[0]?.customer_type || 1))?.rate;
    let maxMetalRate24 = metalAttribute[0]?.values?.find((z:any)=>(z.value.includes("24")) && z.vendor_type == (me?.business_profile[Object.keys(me?.business_profile)[0]]?.customer_type || orderBy(me?.shops,["customer_type"],["desc"])?.[0]?.customer_type || 1))?.rate;
    let maxStoneRate = stoneAttribute[0]?.values?.find((z:any)=>z.value == max_price_stone_attribute?.value && z.vendor_type == (me?.business_profile[Object.keys(me?.business_profile)[0]]?.customer_type || orderBy(me?.shops,["customer_type"],["desc"])?.[0]?.customer_type || 1))?.rate;
    if(minMetalRate)
    {
      ...
      const minWastageValue = Number(min_price_variation?.wastage || 0) + Number(product?.shop?.wastage_markup || 0);
      min_price_variation.wastagePrice = product?.shop?.markup_type === 'p' ?
        Number(Number(minMetalRate24)*Number(Number(min_price_variation?.netWeight)/100*minWastageValue))?.toFixed(2)
        : Number(Number(min_price_variation?.netWeight)*Number(minWastageValue))?.toFixed(2);
        if(min_price_variation?.options && min_price_variation?.options?.length && min_price_variation?.options?.find((x:any)=>["Diamond","Polki","SOLITAIRE","Lab Grown Diamond"].includes(x.name)))
        {
    ...
    if(min_price_variation)
    {
      ...
      if(!me || !(me?.business_profile[Object.keys(me?.business_profile)[0]]?.customer_type || orderBy(me?.shops,["customer_type"],["desc"])?.[0]?.customer_type || me?.customer_type) || ((me?.business_profile[Object.keys(me?.business_profile)[0]]?.customer_type == 1 || orderBy(me?.shops,["customer_type"],["desc"])?.[0]?.customer_type == 1 || me?.customer_type == 1))) // && Difference_In_Days < 30))
      {
        min_price_variation.price = Number(min_price_variation?.metal) + Number(Number(min_price_variation?.stone)*stone_extra) + Number(Number(min_price_variation?.wastagePrice)*wastage_extra) + Number(Number(min_price_variation?.makingCharges*making_extra));  
      }
    }
    if(maxMetalRate)
    {
      ...
      const maxWastageValue = Number(max_price_variation?.wastage || 0) + Number(product?.shop?.wastage_markup || 0);
      max_price_variation.wastagePrice = product?.shop?.markup_type === 'p' ?
        Number(Number(maxMetalRate24)*Number(Number(max_price_variation?.netWeight)/100*maxWastageValue))?.toFixed(2)
        : Number(Number(max_price_variation?.netWeight)*Number(maxWastageValue))?.toFixed(2);
        if(max_price_variation?.options && max_price_variation?.options?.length && max_price_variation?.options?.find((x:any)=>["Diamond","Polki","SOLITAIRE","Lab Grown Diamond"]?.includes(x.name)))
        {
    ...
    if(max_price_variation)
    {
      ...
      if(!me || !(me?.business_profile[Object.keys(me?.business_profile)[0]]?.customer_type || orderBy(me?.shops,["customer_type"],["desc"])?.[0]?.customer_type || me?.customer_type) || ((me?.business_profile[Object.keys(me?.business_profile)[0]]?.customer_type == 1 || orderBy(me?.shops,["customer_type"],["desc"])?.[0]?.customer_type == 1 || me?.customer_type == 1))) // && Difference_In_Days < 30))
      {
        max_price_variation.price = Number(max_price_variation?.metal) + Number(Number(max_price_variation?.stone)*stone_extra) + Number(Number(max_price_variation?.wastagePrice)*wastage_extra) + Number(Number(max_price_variation?.makingCharges)*making_extra);
      }
    }
  ...
  const [attributes, setAttributes] = useState<{ [key: string]: string }>(firstAttributes);
  if(i==0)
  {
  ...
  if (isSelected) {
    ...
    userType = (me?.business_profile[Object.keys(me?.business_profile)[0]]?.customer_type || orderBy(me?.shops,["customer_type"],["desc"])?.[0]?.customer_type || me?.customer_type || 1);
      let valueMetal = {
        name:'',
        value:'',
      };
      ...
      let userMetalRate = userMetalAttribute[0]?.values.find((z:any)=>z.value == valueMetal?.value && z.vendor_type == (me?.business_profile[Object.keys(me?.business_profile)[0]]?.customer_type || orderBy(me?.shops,["customer_type"],["desc"])?.[0]?.customer_type || me?.customer_type || 1))?.rate || 0;
      let userMetalRate24 = userMetalAttribute[0]?.values.find((z:any)=>(z.value.includes("24")) && z.vendor_type == (me?.business_profile[Object.keys(me?.business_profile)[0]]?.customer_type || orderBy(me?.shops,["customer_type"],["desc"])?.[0]?.customer_type || me?.customer_type || 1))?.rate || 0;
      if(userMetalRate)
      {
        selectedVariation.metal = (Number(userMetalRate)*Number(selectedVariation?.netWeight))?.toFixed(2);
        const wastageValue = Number(selectedVariation?.wastage || 0) + Number(product?.shop?.wastage_markup || 0);
        selectedVariation.wastagePrice = product?.shop?.markup_type === 'p' ?
        Number(Number(userMetalRate24)*Number(Number(selectedVariation?.netWeight)/100*wastageValue))?.toFixed(2)
        : Number(Number(selectedVariation?.netWeight)*Number(wastageValue))?.toFixed(2);
        if(selectedVariation?.options && selectedVariation?.options?.length && selectedVariation?.options?.find((x:any)=>["Diamond","Polki","SOLITAIRE","Lab Grown Diamond"]?.includes(x.name)))
        {
      ...
      {
        selectedVariation.price = Number(selectedVariation?.metal) + (Number(selectedVariation?.diamond || 0) || 0) + Number(selectedVariation?.stone) + Number(selectedVariation?.wastagePrice) + Number(selectedVariation?.makingCharges);
        if(!me || (userType == 1))
        {
          selectedVariation.price = Number(selectedVariation?.metal) + Number((Number(selectedVariation?.diamond || 0) || 0)*stone_extra) + Number(Number(selectedVariation?.stone)*stone_extra) + Number(Number(selectedVariation?.wastagePrice)*wastage_extra) + Number(Number(selectedVariation?.makingCharges)*making_extra);
        }
      }
      isLoadingRate = false;
    }
  ...
  return (
    ...
    <div className="col-span-6" style={{ textAlign: 'center', fontWeight: 'bold', marginTop:'15px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'nowrap' }}>
        ...
        <img
          ...
          style={{ maxWidth: '80px', margin: '10px' , marginRight: '20px'}}
        />
      </div>


      {product?.buy_back_policy && product?.buy_back_policy?.buy_back_description && product?.buy_back_policy?.buy_back_value !== 0 && (
  <div className="" style={{ width: "100%", padding: "20px", borderRadius: "10px", boxShadow: "0px 5px 15px rgba(0, 0, 0, 0.2)", display: "flex", alignItems: "center", flexDirection: "column" }}>
    <div style={{ display: "flex", alignItems: "center" }}>
      <p style={{ fontSize: "28px", fontWeight: "bold", margin: "0 10px 0 0", textDecoration: "underline" }}>Buy Back Available</p>
    ...
    <div className="flex">
      <div style={{ marginLeft: "10px",  fontWeight: "normal" }}>
        <p className="">{product?.buy_back_policy?.buy_back_description}</p>
      </div>
    </div>
  </div>
)}
...
<div className="col-span-6 pt-8 lg:pt-0">
        <div className="pb-7">
          ...
          <h2 className="text-heading text-lg md:text-xl lg:text-2xl 2xl:text-3xl font-bold hover:text-black" style={{ color: '#5d6b6b' }}>
            {product?.name}
          </h2>
          <p className="">
          <span className="font-bold" style={{ color: '#d87876' }}>Style Code:</span> {product?.stylecode}
          </p>
  ...
  <span className="font-bold" style={{ color: '#d87876' }}>SPEC:</span>&nbsp;&nbsp;
  {Number(selectedVariation?.netWeight) > 0 && (
    <>
      <span className="font-bold">Net Wt:</span>&nbsp;{selectedVariation?.netWeight} Gms&nbsp;&nbsp;
    </>
  )}
  {Number(
    ...
    <>
      <span className="font-bold">{[2, 3].includes(product?.type?.id) ? 'Diamond Wt:' : 'Stone Wt:'}</span>&nbsp;
      {(
        Number(selectedVariation?.diamondWeight || 0) +
        Number(selectedVariation?.diamondWeight1 || 0) +
        Number(selectedVariation?.diamondWeight2 || 0) +
  ...
  {Number(selectedVariation?.stoneWeight) > 0 && (
    <>
      <span className="font-bold">Stone Wt:</span>&nbsp;{selectedVariation?.stoneWeight} Ct&nbsp;&nbsp;
    </>
  )}
  {Number(selectedVariation?.wastage) > 0 && (
    <>
      ...
      {(() => {
        const wastageValue = ((!me || (userType == 1)) && selectedVariation?.wastage && product?.shop?.wastage_markup) //&& Difference_In_Days < 30))
          ? Number(Number(Number(Number(selectedVariation?.wastage || 0)) + Number(product?.shop?.wastage_markup || 0)) * wastage_extra)
          : Number(selectedVariation?.wastage || 0) + Number(product?.shop?.wastage_markup || 0);
      ...
      >
        ...
        >
          <span>
            ...
            (
              <>
               <span style={{ display: 'inline-flex', alignItems: 'center', fontSize: '18px', fontWeight: 'bold', color: 'white'  }}>
            Hide Customization
            <img
              src="../../assets/images/cus.gif"
              alt="Customization Icon"
              style={{ width: '60px', height: '60px', marginLeft: '-5px', verticalAlign: 'middle', marginTop: '-40px'}}
            />
          </span>
            </>
            )
...
{'\n'}<br />
 ...
 {
 isEmpty(selectedVariation) && !isLoadingRate ? <li></li> :
...
{
                        Number(selectedVariation?.metal) && Number(selectedVariation?.diamond) ?
                      <td></td> : ""
                      }
                      {
...
}
{
                    ...
                    <hr style={{border:"0.1px solid #2f3737" }}/>
                      {
                        round(Number(selectedVariation?.metal))?.toFixed(0) || Number(0)?.toFixed(0)
                      }
                      </td>:""
}

{
                    ...
                    <hr style={{border:"0.1px solid #2f3737" }}/>
                        ...
                        ?
                        round(Number(Number(selectedVariation?.diamond))*stone_extra)?.toFixed(0)
                        :
                        round(Number(selectedVariation?.diamond))?.toFixed(0) || Number(0)?.toFixed(0)
                        }
                        </td> : ""
                      }
                      {
                    ...
                    <hr style={{border:"0.1px solid #2f3737" }}/>
                        ...
                        ?
                        round(Number(Number(selectedVariation?.stone))*stone_extra)?.toFixed(0)
                        :
                        round(Number(selectedVariation?.stone))?.toFixed(0) || Number(0)?.toFixed(0)
                        }
                        </td> : ""
                      }
                      {
                    ...
                    <hr style={{border:"0.1px solid #2f3737" }}/>
                        {
                          ...
                          ?
                          round(Number(Number(selectedVariation?.wastagePrice)*wastage_extra))?.toFixed(0)
                          :
                          round(Number(selectedVariation?.wastagePrice))?.toFixed(0) || Number(0)?.toFixed(0)
                        }
                      </td> : ""
                      }
                      {
                    ...
                    <hr style={{border:"0.1px solid #2f3737" }}/>
                        {
                          ...
                          ?
                          round(Number(Number(selectedVariation?.makingCharges)*making_extra))?.toFixed(0)
                          :
                          round(Number(selectedVariation?.makingCharges))?.toFixed(0) || Number(0)?.toFixed(0)
                        }
                      </td> :""
                      }
                                           {
...
}
{
                    ...
                    <div className={"text-base"}>
                        {
                          ((!me || (userType == 1)) && (selectedVariation?.makingCharges || selectedVariation?.metal || selectedVariation?.stone || selectedVariation?.diamond || selectedVariation?.wastagePrice)) //&& Difference_In_Days < 30))
                          ?
                          ...
                          )/100)*(product?.type?.id < 5 ? 3 : 1.5)))?.toFixed(0) || Number(0)?.toFixed(0)
                          :
                          round(Number(Number(Number(
...
}
{
      ...
      {/* Check if the screen width is greater than or equal to 1025 pixels (desktop view) */}
            {width >= 1025 ? (
              <div style={{ textAlign: 'center', fontWeight: 'bold', marginTop: '15px' }}>
