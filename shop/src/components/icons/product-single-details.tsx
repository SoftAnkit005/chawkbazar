...
const ProductSingleDetails: React.FC<Props> = ({ product }: any) => {
  ...
  if (isSelected) {
    ...
    console.log("admin_commission_rate_customer", admin_commission_rate_customer);

    if (selectedVariation?.stonePrice && selectedVariation.stoneWeight && Number(selectedVariation?.stonePrice) !== 0 && Number(selectedVariation?.stoneWeight !== 0)) {
      const newStonePrice = Number(selectedVariation?.stonePrice) * Number(1 + admin_commission_rate_customer / 100) * Number(selectedVariation?.stoneWeight);
      console.log("newStonePrice", newStonePrice);
      selectedVariation.stone = newStonePrice;
    }
    ...
    console.log("selectedVariation?.metal", selectedVariation?.metal);
    console.log("selectedVariation?.diamond", selectedVariation?.diamond);
    console.log("selectedVariation?.stone", selectedVariation?.stone);
    console.log("selectedVariation?.wastagePrice", selectedVariation?.wastagePrice);
    console.log("selectedVariation?.makingCharges", selectedVariation?.makingCharges);



    const count1 = Number(selectedVariation?.metal) + Number(selectedVariation?.diamond || 0) + Number(selectedVariation?.stone || 0) + Number(selectedVariation?.wastagePrice || 0) + Number(selectedVariation?.makingCharges || 0);
    ...
    selectedVariation.price = count1;
            ...
            <table cellPadding={1} style={{ textAlign: "center" }}>
              ...
              <tr style={{ fontWeight: "bold", color: "black" }}>
                ...
                {Number(selectedVariation?.stone) ? (
                  <td>
                    <hr style={{ border: "0.1px solid #2f3737" }} />
                    {round(Number(selectedVariation?.stone || 0)).toFixed(0)}
                        ...
                        ? round(
                          Number(
                            Number(
                              Number(
                                round(
                                  Number(selectedVariation?.metal) || 0
                                )?.toFixed(0)
                              ) +
                              Number(
                                round(
                              ...
                              Number(
                                round(
                                  Number(selectedVariation?.stone) || 0
                                )?.toFixed(0)
                              ) +
                              Number(
                                round(
