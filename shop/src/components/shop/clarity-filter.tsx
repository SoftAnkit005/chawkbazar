...
export const ClarityFilter = ({changedFilter}: Props) => {
    ...
    useEffect(() => {
      setkeenOptions({
            breakpoints: { "(min-width: 600px)": { slides: { perView: 8, spacing: 10 }, },"(min-width: 1000px)": { slides: { perView: 12, spacing: 10 }, }, },
                        ...
                        <div ref={ref} className="keen-slider">
                            {clarityFilterItems?.map((item,index) => 
                                <>
                                    ...
                                    <label className="keen-slider__slide border border-[#fff] rounded-lg  hover:border-[#24182E] p-2 number-slide1 flex flex-col items-center justify-center py-3" htmlFor={`claritycb${index}`}> <p className="font-semibold text-sm">{item.name}</p> </label>
                                </>
                            )}
                        </div>
                    </div>
