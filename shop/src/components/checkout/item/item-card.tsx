...
const ItemCard = ({ item, notAvailable }: Props) => {
  ...
  const productUrl = `/products/${item.slug}`; // Adjust the URL based on your application's routing
  return (
    <a href={productUrl} className="block hover:underline">
      <div
      ...
      >
        ...
        >
          {!notAvailable ? price.split(".")[0] : t("text-unavailable")}
        </span>
      </div>
    </a>
  );
