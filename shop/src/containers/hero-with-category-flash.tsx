...
import ExtraBanner from "./extra-banner";
        ...
        >
          ...
          <div className="col-span-full lg:col-span-5 xl:col-span-5 row-span-full lg:row-auto grid grid-cols-2 gap-2 md:gap-3.5 lg:gap-5 xl:gap-7">
            {data?.sort((a, b) => a.sequence - b.sequence)
              ?.filter(
                (item: any) => item.bannerType === "vintageDemoGridBanner"
              )
              ?.slice(0, 3)
              ?.map((banner: any) => (
                <BannerCard
                  ...
                  href={`/${banner.slug}`}
                  className={
                    banner.type === "large" ? "col-span-2" : "col-span-1"
                  }
                />
              ))}
            {width <= 999 ? <ExtraBanner data={data} /> : ""}
          </div>
          <SellWithProgressCardSection />
        </div>
        {width > 999 ? <ExtraBanner data={data} /> : ""}
        {/* <ExtraBanner data={data} /> */}
      </div>
    </>
  );
};
...
export function CategoryListCardSection() {
  ...
  const { width } = useWindowSize();
  let { data: categories, isLoading: loading, error } = useCategoriesQuery({
    limit: 15,
    parent: null,
  });

  categories ? categories.data = categories?.data?.filter(x=>!["Diamond","Silver","Gold","Solitaire"].includes(x.name)) : [];
...
export function SellWithProgressCardSection() {
  const { width } = useWindowSize();
  const { t } = useTranslation();
  const { data: products, isLoading: loading, error } = useNonSolitaireProductsQuery({
    limit: 10,
    status:"publish"
  });
