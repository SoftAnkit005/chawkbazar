...
class OrderRepository extends BaseRepository
{
    ...
    protected function calculateShopIncome($parent_order)
    {
        foreach ($parent_order->children as  $order) {
            ...
            $adminCommissionRate = $balance->admin_commission_rate_solitaire;
    ...
    public function createChildOrder($id, $request)
    {
        ...
        foreach ($products as $key => $cartProduct) {
            ...
            {
            $collection = collect($product['variation_options']);
            $variation_option = $collection->firstWhere('id', $cartProduct['variation_option_id']);
            $cartProduct['subtotal'] = $variation_option['metal'] + $variation_option['diamond'] + $variation_option['wastagePrice'] + $variation_option['makingCharges'];
            }
            else
            {
