      ...
      <div className="flex flex-wrap my-5 sm:my-8">
        ...
        <div className="mb-5">
            ...
            >
              ...
              {
                taxData?.taxes.filter((x:any)=>!x.parents && x.id!=initialValues?.id).map((x:any)=>
                  <option value={x.id} selected={x.id == initialValues?.parents}>{x.name}</option>
                )
              }
            </select>
          </div>
          ...
          />
          <Input
            label={`Exclude ${t("form:input-label-state")}`}
            {...register("excludeState")}
            error={t(errors.excludeState?.message!)}
            variant="outline"
            className="mb-5"
          />
          <Input
