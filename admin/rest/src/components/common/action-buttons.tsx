...
type Props = {
  ...
  approveSolitaireCsvButton?: boolean;
  approveBusinessButton?: boolean;
...
const ActionButtons = ({
  ...
  isShopActive,
  approveButton = false,
  approveBusinessButton = false,
  approveSolitaireCsvButton = false,
  ...
  function handleBusinessStatus() {
      openModal("BUSINESS_APPROVE_VIEW", id);
  }
  function handleSolitaireStatus() {
    openModal("CSV_APPROVE_VIEW", {id,shop_id});
}
  return (
    <div className="space-s-5 inline-flex items-center w-auto">
      ...
      {approveButton &&
        ...
        {approveSolitaireCsvButton &&
          <button
            onClick={() => handleSolitaireStatus()}
            className="text-accent transition duration-200 hover:text-accent-hover focus:outline-none"
            title={"Approve Import Csv"}
          >
            <CheckMarkCircle width={20} />
          </button>
        }
