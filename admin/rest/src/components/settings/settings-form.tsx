...
export default function SettingsForm({
  ...
  console.log("isOnisOnisOn", isOn);

  const maintenanceHandler = async (value: any) => {
    await axios
      .put(
        `${process.env.NEXT_PUBLIC_REST_API_ENDPOINT}/shop-under-maintainance/1`,
        {
          type: "maintainance",
          value: value,
        }
  ...
  const getmaintenanceHandler = async () => {
    await axios
      .get(
        `${process.env.NEXT_PUBLIC_REST_API_ENDPOINT}/shop-under-maintainance/1`
      )
  ...
  return (
    <>
      ...
      {me && me?.email === "zweler.web@gmail.com" && (
        <>
          <div className="flex flex-wrap pb-8 border-b border-dashed border-gray-300 my-5 sm:my-8">
            <Description
              title={t("form:Maintenence-Status")}
              details={"Change Maintenence Status"}
              className="w-full px-0 sm:pe-4 md:pe-5 pb-5 sm:w-4/12 md:w-1/3 sm:py-8"
            />
            <Card className="w-full sm:w-8/12 md:w-2/3">
