...
const { data: taxData } = useTaxesQuery();
  ...
  const {
    ...
    defaultValues: {
      ...
      applicableTax: initialValues?.applicableTax?.toString() || "",
  ...
  const onSubmit = async (values: FormValues) => {
    const input = {
      ...
      applicableTax: values.applicableTax,
      settings: {
    ...
    if (!initialValues) {
      if(["GOLD JEWELLERY","DIAMOND JEWELLERY","POLKI JEWELLERY","SILVER JEWELLERY","LOOSE DIAMOND","SOLITAIRES","LABGROWN LOOSE DIAMOND","LABGROWN SOLITAIRES","GEMSTONES","SEMI PRECIOUS BEADS"].includes(values.name || ""))
      {
        return;
      }
      createType({
      ...
      <div className="flex flex-wrap my-5 sm:my-8">
        ...
        <Card className="w-full sm:w-8/12 md:w-2/3">
          <Input
            ...
            readOnly={["GOLD JEWELLERY","DIAMOND JEWELLERY","POLKI JEWELLERY","SILVER JEWELLERY","LOOSE DIAMOND","SOLITAIRES","LABGROWN LOOSE DIAMOND","LABGROWN SOLITAIRES","GEMSTONES","SEMI PRECIOUS BEADS"].includes(getValues("name") || "") || false}
            {...register("name")}
...
<div className="mb-5">
            ...
            {...register("applicableTax")}
              name="applicableTax"
              className="mb-5 px-4 h-12 flex items-center w-full rounded appearance-none transition duration-300 ease-in-out text-heading focus:outline-none focus:ring-0 border border-border-base focus:border-accent "
            >
              {
                taxData?.taxes.filter((x:any)=>!x.parents).map((x:any)=>
                  <option value={x.id} selected={x.id == initialValues?.applicableTax}>{x.name}</option>
                )
              }
            </select>
          </div>
